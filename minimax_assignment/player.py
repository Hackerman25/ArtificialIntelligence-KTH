...
class PlayerControllerMinimax(PlayerController):
    ...
    def alphabeta(self, node, state, depth_to_search, alpha, beta, initial_time, nodes_seen, MaximizingPlayer):
        ...
        if time.time() - initial_time > 0.055:
            ...
            if MaximizingPlayer == True:
                v = -999999
                for i in range(0, len(node.children)):
                    eval, action = self.alphabeta(node.children[i], node.children[i].state, depth_to_search - 1, alpha,
                                                  beta, initial_time, nodes_seen, False)
                ...
                for i in range(0, len(node.children)):
                    eval, action = self.alphabeta(node.children[i], node.children[i].state, depth_to_search - 1, alpha,
                                                  beta, initial_time, nodes_seen, True)
    ...
    def iterative_deepening_search(self, node, nodes_seen, max_depth):
        ...
        try:
            ...
            for depth in range(1, max_depth + 1):
                value, move = self.alphabeta(node, node.state, depth, -float('inf'), float('inf'), initial_time,
                                             nodes_seen, True)
                best_move = move
    ...
    def search_best_next_move(self, initial_tree_node):
        ...
        best_index = self.iterative_deepening_search(initial_tree_node, nodes_seen, 10)
